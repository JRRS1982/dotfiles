...
the dotfiles, the symlink makes this skill available as `/dot-gc`.

## Usage examples
```
/dot-gc                                    Infers ticket from branch/commits/context, generates title
/dot-gc GOLD-1924                          Uses that ticket, generates title
/dot-gc -m "Fix avatar upload"             Infers ticket, uses your title
/dot-gc GOLD-1924 -m "Fix avatar upload"   Uses both explicitly
```

## Notes

 - Claude will never auto-trigger this — it only runs when you explicitly type `/dot-gc`. No surprise commits.
